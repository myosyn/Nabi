...
class KordAutocompleteChecker(val kord: Kord, val interactionsManager: InteractionsManager) {
    ...
    fun checkAndExecute(request: DiscordInteraction, requestManager: RequestManager) {
        ...
        val interactionData =
            InteractionData(request.data.resolved.value?.toDiscordInteraKTionsResolvedObjects(kord, guildId))
        ...
        val command = CommandDeclarationUtils.getApplicationCommandDeclarationFromLabel<SlashCommandDeclaration>(
            interactionsManager,
            commandLabels
        )
            ?: return
        ...
        GlobalScope.launch {
            ...
            when (option) {
                is StringCommandOption -> {
                    val autocompleteExecutor = option.autocompleteExecutor
                        ?: error("Received autocomplete request for ${focusedDiscordOption.name}, but there isn't any autocomplete executor declaration set on the option! Did you update the application command body externally?")
