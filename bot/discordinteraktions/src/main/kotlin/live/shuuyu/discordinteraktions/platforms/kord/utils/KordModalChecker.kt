...
class KordModalChecker(val kord: Kord, val interactionsManager: InteractionsManager) {
    ...
    fun checkAndExecute(request: DiscordInteraction, requestManager: RequestManager) {
        ...
        val modalExecutor =
            interactionsManager.modalExecutors.firstOrNull { it.signature() == modalSubmitDeclaration.parent }
                ?: InteraKTionsExceptions.missingExecutor("modal submit")
        val allComponentsInRequest =
            request.data.components.value ?: error("Missing component list for the modal submit request!")
