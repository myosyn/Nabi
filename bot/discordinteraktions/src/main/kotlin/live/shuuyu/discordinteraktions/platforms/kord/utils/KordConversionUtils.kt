...
import live.shuuyu.discordinteraktions.platforms.kord.entities.messages.KordMessage

/**
 * Converts Kord's Resolved Objects to Discord InteraKTions's Resolved Objects
 */
fun ResolvedObjects.toDiscordInteraKTionsResolvedObjects(
    kord: Kord,
    guildId: Snowflake?
    ...
    val members = this.members.value?.map {
        ...
        it.key to Member(
            MemberData.from(
                ...
                guildId
                    ?: error("Guild ID is null, however there are members present in the resolved objects list! Bug?"),
                it.value
            ),
            user.data,
            kord
